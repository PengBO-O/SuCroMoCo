...
class SupConLoss(nn.Module):
    ...
    def forward(self, outputs, labels):
        ...
        ce_loss = (1 - self.alpha) * self.ce_loss(outputs['predicts'], labels)
        cl_loss = self.alpha * self.nt_xent_loss(normed_cls_feats, normed_cls_feats, labels)
...
class DualLoss(SupConLoss):
    ...
    def forward(self, outputs, labels):
        ...
        # print(normed_label_feats.shape)
        ...
        # print(normed_pos_label_feats)
        ce_loss = (1 - self.alpha) * self.ce_loss(outputs['predicts'], labels)
        cl_loss_1 = 0.5 * self.alpha * self.nt_xent_loss(normed_pos_label_feats, normed_cls_feats, labels)
        cl_loss_2 = 0.5 * self.alpha * self.nt_xent_loss(normed_cls_feats, normed_pos_label_feats, labels)
